...
def compute_bbox(start, end, fps, tube_bbox, frame_shape, inp, image_shape, increase_area=0.1):
    ...
    width = right - left
    height = bot - top
    #print("L T R B: " + str(left) + " " + str(top) + " " + str(right) + " " + str(bot))
    ...
    #height_increase1 = 0
    #height_increase2 = 0
    #print("WI HI: " + str(width_increase) + " " + str(height_increase))
    ...
    #print("L T R B: " + str(left) + " " + str(top) + " " + str(right) + " " + str(bot))
    #print("w h: " + str(width) + " " + str(height))
    #left = left-math.ceil(width_increase1*width)
    #top = top - math.ceil(height_increase1*height)
    #right = right+math.ceil(width_increase2*width)
    #bot = bot + math.ceil(height_increase2*height)
    #print("L T R B: " + str(left) + " " + str(top) + " " + str(right) + " " + str(bot))
    ...
    return f'ffmpeg -i {inp} -ss {start} -t {time} -filter:v "crop={max(w,h)}:{max(w,h)}:{left}:{top}" crop.mp4'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_shape", default=(256, 256), type=lambda x: tuple(map(int, x.split(','))),
                        help="Image shape")
    parser.add_argument("--increase", default=0.215, type=float, help='Increase bbox by this amount')
    parser.add_argument("--iou_with_initial", type=float, default=0.25, help="The minimal allowed iou with inital bbox")
    parser.add_argument("--inp", default=r"F:\PRESERVE_SPACE_PROJECTS\CVPR2022-DaGAN\celebV\raw/cut3.mp4",
